...
import { listBenchmarksFromDb, listMySubmissions } from "@/lib/benchmark/db";
...
export const Route = createFileRoute("/benchmarks")({
  ...
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "Benchmark Tasks — Team SNOVA ESP" },
      {
...
function BenchmarksPage() {
  ...
  const visible = (benchmarks.data ?? []).filter(
    (b) => b.status === "active" && (b.role === "all" || b.role === role),
  );

  const busy = loading || benchmarks.isLoading || player.isLoading;
      ...
      <div className="space-y-6">
        ...
        {busy ? (
          ...
          <div className="rounded-2xl border border-border bg-surface/60 p-8 text-center">
            <div className="font-semibold">No active tasks</div>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            {visible.map((benchmark) => (
              <BenchmarkCard
                key={benchmark.id}
                benchmark={benchmark}
                completed={passedIds.has(benchmark.id)}
                onClick={() =>
                  navigate({
                    to: "/benchmarks/$id",
